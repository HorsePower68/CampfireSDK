...
object ControllerPost {
    ...
    fun showPostPopup(view: View, unit: UnitPost) {
        WidgetMenu()
                ...
                }.condition(ENABLED_COPY_LINK && unit.isPublic)
                ...
                .add(R.string.post_menu_change_tags) { w, card -> ControllerCampfireSDK.onToPostTagsClicked(unit.id, true, Navigator.TO) }.condition(ENABLED_CHANGE_TAGS && unit.isPublic)
                .add(R.string.app_remove) { w, card -> ControllerApi.removeUnit(unit.id, R.string.post_remove_confirm, R.string.post_error_gone) }.condition(ENABLED_REMOVE)
                .add(R.string.app_to_drafts) { w, card -> ControllerUnits.toDrafts(unit.id) { ControllerCampfireSDK.onToDraftsClicked(Navigator.REPLACE) } }.condition(ENABLED_TO_DRAFTS && unit.isPublic)
                ...
                .add(R.string.app_report) { w, card -> ControllerApi.reportUnit(unit.id, R.string.post_report_confirm, R.string.post_error_gone) }.condition(ENABLED_REPORT)
                .add(R.string.app_clear_reports) { w, card -> ControllerApi.adminClearReportUnit(unit.id, R.string.post_clear_reports_confirm, R.string.post_error_gone) }.backgroundRes(R.color.blue_700).condition(ENABLED_CLEAR_REPORTS && ControllerApi.can(unit.fandomId, unit.languageId, API.LVL_MODERATOR_BLOCK) && unit.reportsCount > 0)
                ...
                .add(R.string.post_menu_change_tags) { w, card -> ControllerCampfireSDK.onToPostTagsClicked(unit.id, false, Navigator.TO) }.backgroundRes(R.color.blue_700).condition(ENABLED_MODER_CHANGE_TAGS && ControllerApi.can(unit.fandomId, unit.languageId, API.LVL_MODERATOR_POST_TAGS))
                .clearGroupCondition()
                .add(if (unit.important == API.UNIT_IMPORTANT_IMPORTANT) R.string.unit_menu_important_unmark else R.string.unit_menu_important_mark) { w, card -> markAsImportant(unit.id, !(unit.important == API.UNIT_IMPORTANT_IMPORTANT)) }.backgroundRes(R.color.blue_700).condition(ENABLED_INPORTANT && ControllerApi.can(unit.fandomId, unit.languageId, API.LVL_MODERATOR_BLOCK) && unit.isPublic)
                .groupCondition(!ControllerApi.isCurrentAccount(unit.creatorId) && unit.isPublic)
                .add(R.string.admin_make_moder) { w, card -> makeModerator(unit) }.backgroundRes(R.color.red_700).condition(ENABLED_MAKE_MODER && ControllerApi.can(API.LVL_ADMIN_MAKE_MODERATOR))
                .add(R.string.unit_menu_change_fandom) { w, card -> changeFandomAdmin(unit.id) }.backgroundRes(R.color.red_700).condition(ENABLED_MODER_CHANGE_FANDOM && ControllerApi.can(API.LVL_ADMIN_POST_CHANGE_FANDOM))
                .clearGroupCondition()
                .asSheetShow()
    }

    fun notifyFollowers(unitId: Long) {
        ApiRequestsSupporter.executeEnabledConfirm(R.string.post_create_notify_followers, R.string.app_notify, RPostNotifyFollowers(unitId)) {
    ...
    fun updateSpoilers(listMine: ArrayList<CardPage>) {
        ...
        while (!list.isEmpty()) {
            ...
            if (card is CardPageSpoiler) parseSpoiler(list, (card.page as PageSpoiler).count, !(card.page as PageSpoiler).isOpen)
        }
    }
    ...
    private fun parseSpoiler(list: ArrayList<CardPage>, maxCount: Int, hide: Boolean) {
        ...
        while (!list.isEmpty()) {
            ...
            parsedPages++
            card.setHidedX(hide)
            if (card is CardPageSpoiler) parseSpoiler(list, (card.page as PageSpoiler).count, !(card.page as PageSpoiler).isOpen || hide)
    ...
    fun changeFandom(unitId: Long) {
        ControllerCampfireSDK.SEARCH_FANDOM.invoke { fandom ->
            ApiRequestsSupporter.executeEnabledConfirm(R.string.unit_menu_change_fandom_confirm, R.string.app_change, RPostChangeFandom(unitId, fandom.id, fandom.languageId, "")) {
                ToolsToast.show(R.string.app_done)
        ...
        ControllerCampfireSDK.SEARCH_FANDOM.invoke { fandom ->
            WidgetField()
                    ...
                    .setOnEnter(R.string.app_change) { w, comment ->
                        ApiRequestsSupporter.executeEnabled(w, RPostChangeFandom(unitId, fandom.id, fandom.languageId, comment)) {
                            ToolsToast.show(R.string.app_done)
                            EventBus.post(EventUnitFandomChanged(unitId, fandom.id, fandom.languageId, fandom.name, fandom.imageId))
                        }
                    }
        ...
        WidgetField()
                ...
                .setOnEnter(if (!important) R.string.app_do_unmark else R.string.app_do_mark) { w, comment ->
                    ApiRequestsSupporter.executeEnabledConfirm(
                            ...
                            RFandomsModerationImportant(unitId, important, comment)) { r ->
                        ToolsToast.show(R.string.app_done)
                        EventBus.post(EventUnitImportantChange(unitId, if (important) API.UNIT_IMPORTANT_IMPORTANT else API.UNIT_IMPORTANT_DEFAULT))
                    }
                }
                .asSheetShow()
    }
