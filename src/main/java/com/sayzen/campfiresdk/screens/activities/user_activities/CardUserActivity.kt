...
class CardUserActivity(
    ...
    override fun bindView(view: View) {
        ...
        if (ControllerApi.isCurrentAccount(userActivity.tag_1)) {
            ...
            vButton.setOnClickListener { reject() }
        } else {
    ...
    private fun reject() {
        WidgetReject(userActivity.id).asSheetShow()
    }

    private fun member() {
        ApiRequestsSupporter.executeEnabledConfirm(R.string.activities_relay_race_member_text, R.string.app_participate, RActivitiesRelayRaceMember(userActivity.id, true)) { r ->
            ToolsToast.show(R.string.app_done)
            EventBus.post(EventActivitiesRelayRaceMemberStatusChanged(userActivity.id, 1, r.myIsCurrentMember))
        }
    }

    private fun no_member() {
        ApiRequestsSupporter.executeEnabledConfirm(R.string.activities_relay_race_member_text_no, R.string.app_participate_no, RActivitiesRelayRaceMember(userActivity.id, false)) { r ->
            ToolsToast.show(R.string.app_done)
            EventBus.post(EventActivitiesRelayRaceMemberStatusChanged(userActivity.id, 0, false))
        }
    }
