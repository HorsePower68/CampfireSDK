...
----------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Campfire 0.90 Beta
    Вырезаны форумы, ими никто не пользовался. Вместо форумов введены чаты фэндомов. Работают почти так-же как форумы, но отображаются в списке диалогов. В ленте появились новые вкладки "Лучшее/Хорошее", посты набравшие достаточно кармы будут пропадать из бездны и переходить в эти экраы.

    Изменения:
    - Новые вкладки в ленте "Лучшее/Хорошее"
    - Убраны некоторые настройки ленты из-за изменения логики её работы. (Прости @lololonL)
    - Форумы заменены чатами фэндома.
    - При создании рубрики больше не отображается навигация.
    - В некоторых событиях ссылка на фэндом заменена гиперссылкой, теперь это выглядит намного лучше.
    - Добавлена категория "Места".
    - Добавлен экран анаанимного пользователя (пока не завершен).
    - Больше не будет приходить уведомление о реакции на свой комментарий.
    - В задание написать привет в чате, добавлено поиснение что в чате фэндома.
    - Рубрики теперь отображаются при просмотре поста отдельным экраном.
    - Добавлено несколько фраз при загрузке ленты.
    - Теперь можно поставить анонимную оценку через экран оценок поста.
    - Теперь можно ограничть голосованеи по уровню и карме.
    - Доработаны некоторые анимации приложения, они должны стать плавнее.
    - Для тех кто использует Campfire SDK (да, именно вы, два сумашедших человека), Unit переименован в Publication.

    Исправления:
    - Иногда на беседе отображалась иконка с флагом.
    - Если ставить ананимную оценку, в уведомлении было имя пользователя.
    - На экране ребрики не отображалась рубрика на карточке поста.
    - На экране ребрики не отображался лучший комментарий на карточке поста.
    - При создании страницы с тектом не все кнопки блокировались во время связи с сервером.
    - Админы видили кнопки доступные только создателю поста когда меняли теги.
    - Не скрывалась клавиатура при открытии диалогов.
    - Нельзя было поменять роль пользователя в чате.
    - Не читалось уведомление об упоминании при открытии поста.
    - Иногда можно было отправить несолько стикеров разом.
    - Иногда в профиле отображалось некорректное кол-во оценок.
    - Исправленна ошибка из-за котрой любой пользователь мог стать протоадмином нажав определенную комбинацию клавишь на джойтике.
    - При нажатии кнопки кадрирования на картинке в диалоге комментария, дилог не скрывался при переходе на экран.
    - Нельяз было пройти интро экран (выбор логина), если аккаунт заблокирован.
    - Некоторые админы (по уровню, но не карме), не отображались как модераторы в фэндомах.
    - Можно было многократно открыть один и тот-же чат.
    - Если делать форматированный текст радужного цвета то всё ломалось. (Проблема полностью не решена, но стала менее критичной)
