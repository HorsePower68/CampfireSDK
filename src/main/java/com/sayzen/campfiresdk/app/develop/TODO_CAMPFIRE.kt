...
/*
    ...
    $ Наместники
        -   Отображать как давно намесник

        -   !!! Учитывать только с о дня становления наместником
        -   Достижение стать наместником
        -   Достижение за посты в фэндоме (не много)
        -   Достижение за вики в фэндоме (не много)
        -   Достижение за карму в фэндоме
        -   Достижение за подписчиков в фэндоме
        -   Достижение за ссылки в фэндоме
        -   Достижение за картинки в фэндоме
        -   Достижение за описание в фэндоме
...
----------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Campfire 0.94 Beta
    В битве идей решили что нужны достижения для эстафет - получайте. В целом доработал эстафеты, добавил нескоько полезных функцый. Вернулась видео реклама. Появилась подписка на эстафеты.

    Новая система - наместники. В каждом фэндоме, отдельно для каждого языка может быть назначен наместник. Это человек будет заниматься развитием фэндома, эго описание, контентом и популиризацией. Зачем? Ради 8-ми новых достижений, созданных специально для наместников.

    Изменения:
    - На экране чатов убрано имя того кто писал вам сообщение в приватном чате (из самого сообщения).
    - На экране чатов своё имся заменено на "Вы" (в сообщениях).
    - На экране чатво убраны полоски разделяющие чаты. Так нынче модно.
    - Теперь при написании большого сообщения или комментария скрепка бедет перемещатсья вправо, увеличивая поля вводы на 20-30%.
    - Измененен текст модераторского жействия про важный пост. Теперь там написано "пост" вместо "публикация".
    - Уменьшена сила зимней коробки, теперь с ней можно будет пользоваться приложением.
    - При выборе дополнительных опцый при публикации поста добавлена кнопка для закрытия диалога.
    - Оптимизирована работа с Campfire SDK.
    - Добавлена система наместников и 8 новых достижений для них.
    - Название эстафеты в уведомлении выделено ковычками.
    - Теперь если не создать пост в эстафете в течении 12 часов, придет уведомление об этом.
    - Теперь если вы комуто передали эстафету, вам придет уведомление в случае если ое откажется от неё.
    - Теперь ссылки на чаты и фэндомы по умолчанию копируются с языкм.
    - В приложении снвоа появилась видео реклама.
    - Добавлены достижения для эстафет. (Аж целых 5 штук)
    - Убраны разделители на карточках уведомлений.
    - Возможность создать пост с карточки эстафеты.
    - После выбора дополнительных параметров для поста (рубрика, отложка и т.д.) они будут отображатсья на основном экарне с тегами.
    - На экарне ачивок убраны разделители.
    - Небольшой редизайн экрана привелегий.
    - Теперь можно подписыватсья на эстафеты (получат уведомления о новых постах). У частники будут подписываться автоматически.

    Исправления:
    - При нажатии на уведомление о комментарии небыло скролла к нему.
    - При скачиваннии картинки иногда вылетало приложение.
    - Иногда не срабатывала зимняя коробка.
    - При нажатии на уведомление о сообщении в чате к нему небыло скролла.
    - При нажатии на уведомление об упоминании в чате - приложение вылетало.
    - Иногда пользоватлям несколько раз подряд передавалась эстафета.
    - В чате иногда появлялись дыры между сообщениями.
    - На екоторых экранах с ошибками не работала кнопка назад.
    - В диалоге изменения коофицента фэндома сжимались поля.
    - Иногда нельзя было изменить коофицент фэндома.
    - На некоторых постах, при определенном стичении остоятельств не отображалась кнопка "Показать"
    - Карточка обновления отображалась под квестом.
    - Иногда вылетало приложение при попытке показать диалог.
    - Иногда вылетало при загрузке чата.
    - Не работало форматирование вики ссылок.
    - Просчитаны вероятности, учтены погрешнасти, проработаны риски.
    - Тормозило удаленеи сообщений в чате.
    - Иногда в закладках мог отобразиться один и тот-же пост дважды.
    - Можно было писать в чаты с произвольным id языка.
    - Можно было перейти в чаты с произвольным id языка.
    - Вместо текста комментариев в уведомлениях отображался текст поста.
    - Если в эстафете нет постов, то посреди экрана стафеты была надпись "пусто".
    - Иконка уведомления о эстафете в меню была серой.
    - Не пропадал счетчик эстает после публикации поста.
    - При удален эстафеты не пропадло уведомление.
    - В ведомлениях о назначении и снятии с поста модератора был перепутан текст.
    - Криво работали параметры фэндома и его фонавая картинка. Могли пропасть при редактировании.
    - После смены аккаунта не сбрасывался счетчик сообщений.
    - Если зайти в вики через английский фэндом посты были все равно на русском.
    - Не было перехода к сообщению в чате по уведомлению о ответе.
    - Не было выхода с экрана эстафеты при удалении.
    - Виджет выбора картинки сначала отображался очень маленьким.
