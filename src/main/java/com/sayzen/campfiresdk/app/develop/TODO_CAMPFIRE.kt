...
/*
    !!!!!!!!!!!!! Дефолтная картика 128 для оценок
...
----------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Campfire 0.89 Beta
    В битве идей победили рубрики. Модераторы фэндомов теперь могут их создавать и назначать владельца. Чем чаше и лучше посты в рубрике, тем выше её коофицент кармы и тем больше владелец пулучает кармы за каждый новый пост. Создан фэндом Campfire Hello в котором будут автоматически повляться посты с приветствием новых пользователей (не чаще чем раз в час). Исправлено несколько ошибок. Переработан стартовый экран.


    Изменения
    - Рубрики. Создать и редактировать их могут только модераторы, но владельцем рубрики может быть кто угодно. Коофиценты повышаются в понедельник утром, если хотябы один пост набрал 100 кармы то коофицент увеличится на 0.05, если нет то уменьшится на 0.1, минимум 1 максимум 3. Коэфицент суммируется с коэфицентом фэндома по формуле: коф = кФ + (кР -1), т.е. если у рубрики коф = 2, а у фэндома 3, то в сумме они дадут 4.
    ...
    - Все давно об этом просят, по этому добавляю рекламные посты в сами фэндомы.
    - На экарне бездны больше не будут отображаться посты фэндомов на которые подписн пользователь.
    - Фильтр уведомлений на карму теперь включен по умолчанию. Пришли к выводу что новичков эти уведомления задалбывать не будут.
    - Правила с экрана в настрйоках прилоежния теперь можно копировть.
    - Реакциям на комментариях добавлен оттуп слева.
    - Со всех уведомлений убраны иконки, мне показалось что они бесполезны.
    - Теперь если на ваш комментарий отрегировали, вам придет уведомление.
    - Появилась возможность ставить оценки анонимно, включается в настройках. (Привет @lololonL)
    ...
    - Иногда в черновиках грузились обычные посты.
    - Вылетало если нажать на Enter в поле названия беседы.
    - Не было уведомления что нет доступа к микрофону при попытке записи голосового сообщения.
    - Задание на сообщения асчитывались в беседах.
    - Админ мог удалять свои наказания. (Привет @Leo1337)
    - При быстрой публикации поста (через функцию поделиться) не закрывался диалог созданяи страницы.
    - Иногда при удалении поста находясь на его экране, экран не закрывался.
    - Само по себе выполнялось здание на карму.
    - Исправлен баг, из-за которого тень красной кнопки налазила на описание по её назначению.
    - При ответе в чате ломались ссылки.
    - У некоторых пользователей отображалось фантомное сообщение в чате.
    - Если редактирвоать вики на разных языках, то пропадала статья на другом.
    - После редактирвоания пустой вики, не было перехода на язык на котором она была отредактирована.
...
----------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Настрйока не загружать картинки
        -   Только при wifi
        -   Толкьо при клике
    ...
    Программная потдержка рубрик
        -   Возможность просматривать удаленные рубрики
        -   Возможность востанавливать удаленные рубрики
        -   Уведомление что рубрика востановлена
