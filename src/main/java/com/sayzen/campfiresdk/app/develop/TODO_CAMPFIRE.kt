...
----------------------------------------------------------------------------------------------------------------------------------------------
    ...
    - Добавлено несколько новых фраз в ленту
    - Добавлено несколько новых фраз в скрытое меню
    - При отправке сообщений больше не будет появляться карточка с загрузкой. (не сразу)
    - Уменьшен размер скриншота в комментариях.
    - Увеличена скорость анимации панели навигации.
    - Редиайн экрана создания текста для поста.
    - Доработаны экраны создания голосования и таблицы.
    - При добавлении ссылки в профиль теперь будет происходить проверка её корректностию

    Исправления:
    - Исправлен (Возможно) мега баг с залипанием приложения при переходе в ленту.
    ...
    - Иногда при отпрвке сообщенйи было видно кнопку "повторить" на карточке с загрузкой.
    - Не окрашивался колокол в чате после перезахода.
    - Можно было перенести пост в тот-же фэндом.
    - Иногда задание заработать карму на посте или комменте выполнялось само по себе.
    - Не отображалось предупреждение при редактировании опубликованного поста, что все изменения отображаются сразу.
    - Отображался некорректный пользователь на событиях администрирования в профиле.
    - Было плохо видно иконки и тектс на светлом фоне в фэндомах и профиле.
    - Не всегда загружлось количество уведомлений/сообщений после пробелм с сетью.
    - Нельяз было перейти в профиль пользователя через уведомление об упоминании.
    - Возможно было просматривать комментарии заблокированных стикерпаков.
    - Редко приложение падало при загруке списока чатов.
    - Изредка приложение падало при закрытии.
    - Время от времени приложение падало при кадрировании изображения.
    - Нестабильно приложение падало при загрузке ленты.
    - Иногда приложение падало при загрузке картинки по ссылке.
    - Бывало, что приложение падало при вставке медиа через клавиатуру.
    - Не регулярно (3 раза) приложение падало от скуки.
