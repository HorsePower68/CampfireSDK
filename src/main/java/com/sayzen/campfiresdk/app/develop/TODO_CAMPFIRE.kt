...
/*
    ...
    !!! Карточка коофицента
    Баг. Иногда после первой авторизации нет токена уведомлений
...
----------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Campfire 0.86 Beta
    Лента разделена на два экрана, в первом будут отображаться подписки во втором все подряд. Вместо кодов языков, теперь отображаются флаги. Добавлена новая информация в профиль пользователя. История публикации. Короче, все ниже:
    ...
    - Добавлена эксперементальная аналитика. Теперь некоторые действия пользователей отслеживаются протоадминами.
    - На экран создания текста для поста добавлена кнопка для быстрого добавления ссылки.
    - В диалоге выбора стикера теперь будет отображатсья загрузка.
    - В диалоге выбора стикера теперь будет отображатсья кнопка повторной загрузки в случае ошибки.
    - В профиле уменьшен разме карточки события о том, что добавлен стикер.
    - Теперь все пользователи могут смотреть что делают админы через кнопку в настройках.
    - В админское событие о изменении коофицента добавлено название фэндома
    - Теперь на модераторских действиях будет отображаться причина по которой оно отклонено.
    - Теперь при выполнении задания будет приходить уведомление об этом, в уведомлении будет указано какое задание выполнено.
    - При выполнении задания в профиле будет появляться карточка с информацией об этом.
    - В режиме не беспокоить в определенное время, приложение будет отображать уведомления, но не будет воспроизводить звук.
    ...
    - Иногда приложение падало при выходе.
    - Можно было изменить коофицент на тот-же самый.
    - Небыло проверки корректности данных сразу при появлении диалога.
    - Небыло перехода к фэндому через уведомление о отмене мод. действия.
    - Небыло перехода к мод действию через уведомление о отмене мод. действия.
    - В прфоиле пользователя написано что он востановил свою публикацию. (должно быть у админа)
    - Форматированная сылка могла сожрать часть текста.
    - Иногда падало при кадрировании картинки.
    - Иногда падало при прослушивании аудиосообщения.
    - Иногда падало при смене экрана.
...
----------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Закрытые посты
        -  Закрывать при создании
        -  Закрывать тложенные
        -  Закрывать опубликованные
        -  Открывать опубликованные
        -  Закрывать опубликованные АДМИН (+события)
        -  Открывать опубликованные АДМИН (+события)

    Сделать нормальную проверку версии
